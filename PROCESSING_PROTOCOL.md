...
11. **Gutenberg:** Publishing Format Auditor (Formatting, Output Verification).

## Data Flow & Project Isolation
*   **Project Isolation:** Every project must have its own dedicated directory (e.g., `/Neon_Veil/`, `/Freehaven_Online/`). Do not mix files.
*   **Inputs:** `/input_subs/` (Source of Truth) or Project Input Folder.
*   **Outputs:** Standardized subfolders within the project directory:
    *   `/doc/` (Legacy .doc)
    *   `/docx/` (Standard .docx)
    *   `/pdf/` (Immutable .pdf)
    *   `/txt/` (Pure .txt)
*   **Context:** `/context/` (Source of Truth for Beats, Kinship, Glossary)
*   **Archive:** Move outdated drafts to `/output_subs/old/` or Project Archive.

## Publication & Artifact Generation (MANDATORY)
**Rule:** At the end of every content generation or revision loop, the agent MUST run `tools_publish_manuscript.py` on the modified Markdown files to ensure the `output/` directory is synchronized.
*   **Command:** `python3 tools_publish_manuscript.py --file [Project_Dir]/[Chapter_File].md`
*   **Frequency:** Immediate execution after saving the source Markdown file.
*   **Verification:** Ensure `.doc`, `.docx`, `.pdf`, and `.txt` files appear in the project's `output/` directory with updated timestamps.

## Consolidated Logging Protocol (Global Mandate)
**Rule:** There shall be only ONE log file per project for all process loops.
*   **File Naming:** `reports/[Project_Name]_Process_Log.md`
*   **Structure:**
    *   **Header:** `# Project: [Name] - Task: [Description] - Date: [Start Date]`
    *   **Loop Entry:** `## Loop [N]: [Action Name]`
    *   **Content:**
        *   **Agent:** [Name]
        *   **Date:** [YYYY-MM-DD]
        *   **Action:** Summary of tasks performed.
        *   **Feedback:** QC/Supervisor comments.
        *   **Response:** Agent's reaction and plan for the next loop.
*   **Prohibition:** Do NOT create separate files for each loop (e.g., `ADVERSARIAL_LOOP_CHAPTER_1.md`). Append to the master log.

## Revision Loops (Feedback Integration)
1.  **Submission:** Agents submit work.
2.  **QC/Supervisor Review:** Feedback is provided.
3.  **Loop Initiation:**
    *   Agent acknowledges feedback in the `Process_Log.md`.
    *   Agent performs revisions.
    *   **Gutenberg** verifies formatting and output generation.
4.  **Final Approval:** Only after QC approves does the project move to "Published" status.
...
1.  **Engage Janus:**
    ...
    *   **Rename:** Must append a timestamp (e.g., `_YYYYMMDD_HHMM.md`). **NEVER** overwrite an existing file in `depreciated/`.
...
*   **Linh** prevents "register collapse" by detecting scene mode (Public/Private).
...
## Master Narrative Log (Context Preservation)
**Mandate:** Every project must have a `MASTER_NARRATIVE_LOG.md` located in its context folder.
*   **Purpose:** To provide a detailed, cumulative summary of the *actual* narrative events as written.
*   **Update Rule:** This log must be updated after every creative writing session.
*   **Content:**
    *   Scene summaries.
    *   Specific dialogue beats or character decisions.
    *   Items acquired/lost.
    *   World state changes.
*   **Prohibition:** Never delete past entries unless the corresponding text is removed from the manuscript. This log represents the "current truth" of the story.
